...
public class RainbowduinoMessage implements RCodes {
    ...
    int[] data = new int[64 + HEADER_LENGTH]; // 4 for HEADER: type, receiver,
                                              // command, dataLength
    ...
    public void consume(int dataByte) {
        if (ready())
            return;
        if (writeIndex == 0
                && !(dataByte == COMMAND || dataByte == REQUEST
                        || dataByte == RESPONSE || dataByte == ERROR))
            return;
    ...
    public boolean ready() {
        return (writeIndex >= HEADER_LENGTH)
                && (writeIndex == (HEADER_LENGTH + data[INDEX_LENGTH]));
    }
    ...
    public String toString() {
        return "TYPE: " + type() + " Receiver: " + receiver() + " Command: "
                + data[INDEX_COMMAND];
    }
