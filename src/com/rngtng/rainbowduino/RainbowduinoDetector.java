...
public class RainbowduinoDetector extends Thread {
    ...
    // private HashMap
    private final Vector<Rainbowduino> rainbowduinos;
    private final Vector<String> checked_ports;
    private final Vector<RainbowduinoListener> listeners;
    ...
    public static void start(PApplet _app) {
        if (rainbowduinoDetector == null)
            RainbowduinoDetector.init(_app);
        if (rainbowduinoDetector.running)
            return;
    ...
    public static void halt() {
        if (rainbowduinoDetector == null || !rainbowduinoDetector.running)
            return;
        rainbowduinoDetector.running = false;
    }

    public static String getAvailablePortName(PApplet _app) {
        Rainbowduino rainbowduino = RainbowduinoDetector.init(_app)
                .detectRainbowudino();
    ...
    public static void notifyError(Rainbowduino rainbowduino) {
        if (rainbowduino == null || rainbowduinoDetector == null)
            return;
    ...
    RainbowduinoDetector(PApplet _app) {
        this.app = _app;
        PApplet.println("Rainbowduino Version MS " + RCodes.API_VERSION_NR);
    ...
    /**
     * Adds a listener who will be notified each time new Rainbowduino is
     * connected
     * 
     * @param listener
     *            the listener to add.
     ...
     * connected
     */
    public static boolean addListener(PApplet _app,
            RainbowduinoListener listener) {
     ...
     * Removes a given listener.
     */
    public static boolean removeListener(PApplet _app,
            RainbowduinoListener listener) {
        return RainbowduinoDetector.init(_app).addListener(listener);
    }
    ...
    public Rainbowduino detectRainbowudino() {
        ...
        for (int i = 0; i < ports.length; i++) {
            if (checked_ports.contains(ports[i]))
                continue; // it's already checked
            checked_ports.add(ports[i]);
            if (PApplet.match(ports[i], "tty") == null)
                continue;
    ...
    public void registerRainbowduino(Rainbowduino rainbowduino) {
        if (rainbowduino == null || rainbowduinos.contains(rainbowduino))
            return;
