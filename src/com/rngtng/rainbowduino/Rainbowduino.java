...
public class Rainbowduino implements RCodes {
    ...
    /**
     * Create a new instance to communicate with the rainbowduino. Make sure to
     * (auto)init the serial port, too
     * 
     * @param _app
     *            parent Applet
     */
    ...
    /**
     * 
     * @return wheter ping was successfull TODO: add time??
     */
    public boolean ping() {
        try {
            receive(PING);
     ...
     *            buffer data
     * @param check
     *            wheter to perform sensity check
     */
    ...
    public int[] bufferGetAt(int adr) {
        try {
            int size = receive(BUFFER_GET_AT, adr);
    ...
    private void sendCommand(int commandCode) {
        ...
        this.master.sendMessage(RainbowduinoMessage.COMMAND, slaveNr,
                commandCode, null);
    }
    ...
    private void sendMessage(int type, int receiverNr, int commandCode,
            int[] data) {
        send(type);
        send(receiverNr);
        send(commandCode);
    ...
    private int receive(int commandCode, int param) throws RainbowduinoError {
        ...
        try {
            sendMessage(RainbowduinoMessage.REQUEST, slaveNr, commandCode,
                    (param > -1) ? new int[] { param } : null);
            waitForMessage();
            PApplet.println("message done");
            if (this.responseMessage.isError())
                throw new RainbowduinoError(responseMessage.param());
            if (this.responseMessage.isResponse()) {
                if (responseMessage.is(commandCode))
                    return responseMessage.paramRead();
            }
    ...
    public String getPortName() {
        if (isConnected())
            return serialPort.port.getName();
        return null;
    }
     ...
     *            port to open
     * @param check
     *            whether to perform valid checks
    ...
    private int waitForMessage(int timeout) throws RainbowduinoTimeOut {
        ...
        while (timeout > 0 && responseMessage == null) {
            ...
            timeout--;
            if (timeout == 0)
                throw new RainbowduinoTimeOut();
        }
        return 1;
    }

    public void messageAvailable(RainbowduinoMessage _message) {
        PApplet.print("R " + slaveNr + " got Message: " + _message.toString());
        if (_message.is(SLAVE_NEW)) {
            PApplet.println(" - registered");
            Rainbowduino newRainbowduino = new Rainbowduino(this,
                    _message.param());
            // if( newRainbowduino.ping() )
            RainbowduinoDetector.init(this.app).registerRainbowduino(
                    newRainbowduino);
            return;
        }
        if (_message.receiver() != slaveNr) {
            PApplet.println(" - forwarded");
            RainbowduinoDetector.init(this.app).messageAvailable(_message);
            return;
        }
        PApplet.println(" - consumed");
